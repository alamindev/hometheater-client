<template>
  <div class="lg:pl-5">
    ...
    <div
      class="bg-white p-5 -mt-4 relative rounded-tl-lg rounded-bl rounded-br shadow-lg"
    >
      <div class="pt-5">
        <nuxt-link
          :to="`/blogs/${post.slug}`"
          class="inline-block pb-4 text-lg text-brand-dark-gray font-semibold hover:underline"
        >
          <h2>{{ post.title }}</h2>
        </nuxt-link>
        <p
          class="font-roboto text-base text-brand-gray leading-relaxed"
          v-html="post.details"
        ></p>
      </div>
