  ...
  <div class="user--product">
    ...
    <section class="bg-white shadow-lg p-5 mt-5 rounded-md">
            ...
            <table class="table w-full">
              ...
              <tbody>
                <tr v-for="product in products" :key="product.id">
                  ...
                  <td class="px-3 py-5">
                    <div
                      v-for="(ser, index) in product.service_type"
                      :key="index"
                    >
                  ...
                  <td class="px-3 py-5">
                    <div class="flex justify-end">
                      ...
                      <nuxt-link
                        :to="`/users/product/${product.id}/details`"
      ...
      <div class="py-5 flex justify-center" v-show="isBooking">
        <Paginator @NextData="NextData" :datas.sync="product_paginator" />
      </div>
    </section>
  </div>
</template>
...
export default {
  ...
  computed: {
    products() {
      return this.$store.state.users.product.products.data;
    },
    product_paginator() {
      return this.$store.state.users.product.products;
    },
    quantity() {
      return (quantity) => {
        if (quantity)
  ...
  methods: {
    async NextData(pageNum) {
      ...
      this.$store.dispatch("users/product/fetchProductPage", pageNum);
    },
    async loaded() {
      await this.$store.dispatch("users/product/fetchProducts");
    },
    async loadedwithparam(param) {
      await this.$store.dispatch("users/product/fetchProductPage", param);
    },
    DeleteOrder(id) {
      this.$swal({
